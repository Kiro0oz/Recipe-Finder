...
function populateRecipeDetail(recipe) {
    ...
    container.querySelector('.detail-author').innerHTML = `<span class="span">by</span> Admin`;

    container.querySelectorAll('.stats-item')[0].querySelector('.display-medium').textContent = recipe.ingredients.length;
    container.querySelectorAll('.stats-item')[1].querySelector('.display-medium').textContent = recipe.cookTimeMinutes;
    container.querySelectorAll('.stats-item')[2].querySelector('.display-medium').textContent = recipe.caloriesPerServing;

    const tagList = container.querySelector('.tag-list');
    tagList.innerHTML = ''; 
    [...recipe.tags, ...recipe.mealType, recipe.cuisine].forEach(tag => {
    ...
    // Servings
    container.querySelector('.ingr-title .label-medium').textContent = `for ${recipe.servings} Servings`;
    ...
    if (recipe.instructions && recipe.instructions.length > 0) {
      const instructionsHTML = recipe.instructions.map(step => `<li class="ingr-item">${step}</li>`).join('');
