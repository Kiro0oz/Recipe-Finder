...
document
  ...
  .addEventListener("click", function (e) {
    ...
    const searchInput = document
      .querySelector("[data-search-field]")
      .value.trim();

    if (searchInput) {
      window.location.href = `/Recipe-Finder/Pages/recipies.html?q=${encodeURIComponent(
        searchInput
      )}`;
    }
  });
...
if (tabBtns.length > 0 && tabPanels.length > 0) {
  ...
  addEventOnElements(tabBtns, "click", function () {
    ...
    const currTabPanel = document.querySelector(
...
function displayMealTypeRecipes(recipes) {
  ...
  mealTypes.forEach((meal) => {
    ...
    const mealRecipes = recipes
      .filter((r) => r.mealType?.some((m) => m.toLowerCase() === meal))
      .slice(0, 5);
...
function createRecipeCard(recipe) {
  return `
    ...
    recipe.name
  }" width="200" height="200" loading="lazy" class="img-cover" />
        </figure>
        <div class="card-body">
          <h3 class="title-small">
            <a href="Pages/detail.html?id=${recipe.id}" class="card-link">${
    recipe.name
  }</a>
          </h3>
          <div class="meta-wrapper">
            ...
            <button class="icon-btn has-state ${
              recipe.is_favorited ? "saved" : "removed"
...
function createTagBadge(tag) {
  return `
    <a href="/Recipe-Finder/Pages/recipies.html?tag=${encodeURIComponent(
      tag
